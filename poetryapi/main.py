...
def get_stores_for_employee(
    phone_number: str = Depends(validate_employee_phone), db: Session = Depends(get_db)
):
...
def get_orders_for_customer(
    phone_number: str = Depends(validate_customer_phone), db: Session = Depends(get_db)
):
    ...
    if updated_order.store_id:
        if updated_order.store_id != store_by_customer.id:
            raise HTTPException(
                ...
                detail="Customer can update order only for his store",
            )

        if store_by_executor is None or updated_order.store_id != store_by_executor.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Customer can update order with executor prescribed only for his store",
            )
    else:
...
def create_order_for_customer(
    ...
    if order.store_id != store_by_customer.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create order only for his store",
        )
    ...
    if store_by_executor is None or order.store_id != store_by_executor.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create order with executor prescribed only for his store",
        )
...
def create_visit_for_customer(
    ...
    if visit.store_id != store_by_customer:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create visit only for his store",
        )
    ...
    if customer_order is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create visit only for his order",
        )
    ...
    if order.visit:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create visit for his order that doesn't already have visit",
        )

    if order.executor != visit.executor_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Customer can create visit with executor prescribed only for his order",
        )
...
def update_visit_for_customer(
    ...
    if updated_visit.store_id:
        if updated_visit.store_id != store_by_customer.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Customer can update visit only for his store",
            )
